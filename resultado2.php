...
while($row = mysql_fetch_assoc($resultado))
{
  ...
  else if(($row["curso1"]=="adm")&&($cont_adm<$admv))
  {
    if(($row["publica"]==0)&&($cont_porcent_admin<9))
    {
       ...
       mysql_query("UPDATE `aluno` SET `matriculado` = 'Administração de Empresas' WHERE `aluno`.`id` = ".$row["id"].";");
       $cont_adm++;
  ...
  else if(($row["curso1"]=="edf")&&($cont_edf<$edfv))
  {
    if(($row["publica"]==0)&&($cont_porcent_edf<=9))
    {
       ...
       mysql_query("UPDATE `aluno` SET `matriculado` = 'Edificações' WHERE `aluno`.`id` = ".$row["id"].";");
       $cont_edf++;
